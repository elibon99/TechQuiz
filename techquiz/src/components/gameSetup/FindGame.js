...
const FindGame = ({auth, addToQueue, restoreRedirectTo, userStats, user, matchQueue, createFriendGame, friends, setUsername, restoreRedirectToFriendGame, friendGameStatus}) => {
    if(!auth.uid) {
    ...
    return(
        userStats ?
            <div className="container find-game-margin">
                ...
                <QuickMatch friends={friends} createFriendGame={createFriendGame} friendGameStatus={friendGameStatus} restoreRedirectTo={restoreRedirectToFriendGame}/>
            </div> :
